...
| 2026-08-01 | **xemu source** (`master` @ `cbffb57d08`) — first run against a codebase, and the first blind one | surveyor + cartographer | ✅ 35 raised, **35 out**, counted in the index's own read-back | ✅ no keyed row asserted-and-false; 2 **off-key counts wrong** — 14 vs **15** workflow files, "12" vs **13** org repos | not run | 🛑 **FAIL at the must tier — 1 HIT / 3 MISS of 4.** Overall **4 HIT / 17 MISS / 0 WRONG** of **21** rows (⚠ the key calls itself "17 rows"; it has 21 — recounted). T-should 2/8, T-deep 1/9. 🎯 **D8, the CI negative, HIT and _promoted_** — a bare "No.", corroborated 2 independent ways, raised to Corrections owed. 🛑 **A1, which thread renders, MISS** — it would **not** have prevented s004's wrong-thread error | ✅ **Found `xemu-project/xemu-test`, an org-level accuracy harness that 13 sessions of the source exercise never saw** → `F019`. 🎯 It was the map's own **weakest-flagged** entry, and the resolver it wrote down was **one `gh api` call** — whose output also contains `nxdk_pgraph_tests`, the T-must row (D1) it missed | Scored 2026-08-01 (s014): **`SCORE-xemu-architecture-2026-08-01.md`**. Output: 12 records, ~1,360 lines, at `the-xemu-cartographer/map_fresh/`. ✅ **Read-only safeguard HELD** — clean `git status`, no new branches, reflog shows only the human's own `checkout` + `pull`. **Verdict: zero fabrication, honest blanks — it stops at the edge of gaps it correctly identified and priced** |

| 2026-08-01 | **xemu source** (`master` @ `cbffb57d08`) — **run 3**, same subject, same key | surveyor + cartographer | ✅ **30 `not_determined` in / 30 out**, hand-counted per record file with the tally shown | ⚠ **2 verified false off-key**: `subprojects/*.wrap` **35 vs 36**; 🛑 `hwxbox-3` "registers 5 SMBus peripherals" — the source is an `if/else` on `video-encoder`, **max 3 in any config**, a *flattened conditional*, not a miscount. ✅ Run 2's two errors both now correct (15 workflows, 12 org repos) | not run | ⚠ **6 HIT / 15 MISS / 0 WRONG** of 21 (**+2** vs run 2). **Still fails at the must tier — 2 of 4.** T-should 3/8, T-deep 1/9. ✅ **C1 fixed** (`debug.h:71-122`, the counter X-macro — run 2's highest-value miss). ⚠ **D1 HIT is gifted** — `BRIEF.md` pre-seeded the org listing incl. `nxdk_pgraph_tests`. 🛑 **A1/A3/B3 all REGRESSED** from "near" to absent — no thread, lock or surface-path claim exists at all | 🎯 **`abaire/xemu-dev_pgraph_test_results`** — a **per-PR accuracy workflow**, `on: pull_request`, `ubuntu-latest`, no secrets, live on **11 xemu PRs** over 10 months, **32** result branches → `F021`. 🛑 **The map listed it as a lead with a one-call resolver and never ran it** | Scored 2026-08-01 (s015): **`SCORE-xemu-architecture-2026-08-01-run3.md`**. 🛑 **NOT a clean replication** — the brief pre-seeded ground truth and it surveyed `~/git_repos/xemu`, both against the runbook. ✅ **Cheap-band gate FIRED** (human approved 11 resolvers → 4 index entries superseded) — clearest fix-works result yet. 🛑 **It does not cover `unexpected:` leads**, which is how F021 was nearly lost |
